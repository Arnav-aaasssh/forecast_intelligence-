...
// NEW: Shared State & Enterprise Model Champion Workspace
...
function nav(pageId) {
    ...
    if(pageId === 'mc') { setTimeout(() => { renderModelChampion(); }, 60); }
    if(pageId === 'bc') renderBusinessContext();
    if(pageId === 'sa' && window.CURRENT_FILTERS) {
        const node = window.CURRENT_FILTERS.nodeName || 'Global';
        const lvl = window.CURRENT_FILTERS.levelName || 'Global';
        if (typeof window.triggerDeepDive === 'function') {
            window.triggerDeepDive(node, lvl);
        } else {
            renderTrendPanel(node, lvl, window.CURRENT_FILTERS.filters || {});
        }
        renderQueueFlatTable(node, lvl, window.CURRENT_FILTERS.filters || {});
    }
}
...
function updateBadgeAndApply(col, checkboxes, btnText, displayName) {
    ...
    if (checked.length > 0) {
        btnText.innerHTML = `${displayName} <span class="custom-select-badge">${checked.length}</span>`;
    } else {
        btnText.innerHTML = displayName;
    }
    applyAllFilters();
...
function resetAllCustomSelects() {
    ...
    gfIds.forEach(col => {
        ...
        const btnText = container.querySelector('.btn-text');
        if (btnText) btnText.innerHTML = col.replace('_', ' ');
    });
}
...
function renderRow(tbody, data, nodeName, levelName, indentLevel, accumulatedFilters, overrideBg = '') {
    ...
    // ALWAYS use the true aggregation stats (works for N>1 groups AND N=1 individual queues)
...
function toggleMCDocPanel() {
    const body = document.getElementById('mc-doc-body');
    const arrow = document.getElementById('mc-doc-arrow');
    ...
    if (body.style.display === 'none' || !body.style.display) {
        ...
        arrow.textContent = '▼ Show Details';
    }
}
...
function renderModelChampion() {
    console.log('Rendering Model Champion Workspace for', window.CURRENT_FILTERS);
    ...
    // Update Scope Badge
    const scopeBadge = document.getElementById('mc-scope-badge');
    let scopeText = 'Global';
    if (levelName !== 'Global') {
        scopeText = `${levelName}: ${nodeName}`;
    ...
    window.CURRENT_MC_RESULTS = results;
    
    if (scopeBadge) {
        scopeBadge.textContent = `Scoped to: ${scopeText} (${results.length} models)`;
    }
    ...
    if (tbody) {
        ...
        results.forEach((r, idx) => {
            const tr = document.createElement('tr');
            const wapeVal = parseFloat(r.WAPE) || 0;
            const wapeBg = wapeVal < 10.0 ? '#E6F4F1' : '#FEF3C7';
            const wapeFg = wapeVal < 10.0 ? '#047857' : '#B45309';
            
            const biasVal = parseFloat(r.Bias) || 0;
            const biasBg = biasVal > 0 ? '#FEF3C7' : '#EFF6FF';
            const biasFg = biasVal > 0 ? '#B45309' : '#1D4ED8';
            const biasSign = biasVal > 0 ? '+' : '';
            
            let rankBadge = `${idx + 1}`;
            if (idx === 0) rankBadge = `<span style="background:#FEF3C7; color:#B45309; padding:2px 8px; border-radius:4px; font-weight:800; font-size:11px;">🥇 #1</span>`;
            else if (idx === 1) rankBadge = `<span style="background:#F1F5F9; color:#475569; padding:2px 8px; border-radius:4px; font-weight:800; font-size:11px;">🥈 #2</span>`;
            else if (idx === 2) rankBadge = `<span style="background:#FFEDD5; color:#C2410C; padding:2px 8px; border-radius:4px; font-weight:800; font-size:11px;">🥉 #3</span>`;
            
            tr.innerHTML = `
                <td style="padding:10px 14px;">${rankBadge}</td>
                <td style="padding:10px 14px; font-weight:600; color:var(--navy);">${r.Model}</td>
                <td class="num" style="font-weight:700; padding:10px 14px; color:var(--navy);">${r.CompositeScore}</td>
                <td class="num" style="background:${wapeBg}; color:${wapeFg}; font-weight:600; padding:10px 14px;">${r.WAPE}%</td>
                <td class="num" style="padding:10px 14px;"><span style="background:${biasBg}; color:${biasFg}; font-weight:600; padding:3px 8px; border-radius:4px; font-size:12px;">${biasSign}${r.Bias}%</span></td>
                <td class="num" style="padding:10px 14px; color:#334155;">${r.MaxErr}%</td>
                <td class="num" style="padding:10px 14px; color:#334155;">${r.Hit10}%</td>
                <td class="num" style="padding:10px 14px; color:#64748B;">${r.ValidWeeks}</td>
            `;
            tbody.appendChild(tr);
        });
    }
    ...
    if (results.length > 0) {
        ...
        if (champEl) champEl.textContent = champ.Model;
        
        const scoreEl = document.getElementById('mc-kpi-score');
        if (scoreEl) scoreEl.textContent = champ.CompositeScore;
        
        // Compute Median
        const scores = results.map(r => parseFloat(r.CompositeScore)).sort((a,b)=>a-b);
        const mid = Math.floor(scores.length / 2);
        const median = scores.length % 2 !== 0 ? scores[mid] : (scores[mid-1] + scores[mid]) / 2;
        
        const scoreSubEl = document.getElementById('mc-kpi-score-sub');
        if (scoreSubEl) {
            scoreSubEl.textContent = `#1 of ${results.length} models · Pop. Median: ${median.toFixed(1)}`;
        }
        
        if (results.length > 1) {
            const runnerUp = results[1];
            const gap = (parseFloat(champ.CompositeScore) - parseFloat(runnerUp.CompositeScore));
            const gapStr = gap.toFixed(1);
            
            const champSubEl = document.getElementById('mc-kpi-champ-sub');
            if (champSubEl) {
                champSubEl.textContent = `+${gapStr} pts over ${runnerUp.Model}`;
            }
            
            const marginValEl = document.getElementById('mc-kpi-margin-val');
            if (marginValEl) marginValEl.textContent = `${gapStr} pts`;
            
            const marginBadgeEl = document.getElementById('mc-kpi-margin-badge');
            let marginLabel = 'Narrow margin';
            let marginBg = '#FEF3C7';
            let marginFg = '#B45309';
            let confTagText = 'Statistical Confidence: Moderate (Error distributions overlap)';
            
            if (gap >= 5.0) {
                marginLabel = 'Strong lead';
                marginBg = '#ECFDF5';
                marginFg = '#047857';
                confTagText = 'Statistical Confidence: High (Clear separation)';
            } else if (gap >= 2.0) {
                marginLabel = 'Clear lead';
                marginBg = '#E6F4F1';
                marginFg = '#0D9488';
                confTagText = 'Statistical Confidence: High-Moderate';
            } else if (gap >= 0.5) {
                marginLabel = 'Moderate margin';
                marginBg = '#FEF3C7';
                marginFg = '#B45309';
                confTagText = 'Statistical Confidence: Moderate (0.5-2.0 pt gap)';
            } else {
                marginLabel = 'Narrow margin';
                marginBg = '#FFE4E6';
                marginFg = '#E11D48';
                confTagText = 'Statistical Confidence: Low-Moderate (Investigate before deploy)';
            }
            
            if (marginBadgeEl) {
                marginBadgeEl.textContent = marginLabel;
                marginBadgeEl.style.background = marginBg;
                marginBadgeEl.style.color = marginFg;
            }
            
            const confTagEl = document.getElementById('mc-stat-confidence-tag');
            if (confTagEl) {
                confTagEl.textContent = confTagText;
                confTagEl.style.background = marginBg;
                confTagEl.style.color = marginFg;
            }
            
            // Decision Synthesis Narrative
            const decTextEl = document.getElementById('mc-decision-text');
            if (decTextEl) {
                const champWape = parseFloat(champ.WAPE);
                const runnerWape = parseFloat(runnerUp.WAPE);
                const champHit = parseFloat(champ.Hit10);
                const runnerHit = parseFloat(runnerUp.Hit10);
                const champBias = Math.abs(parseFloat(champ.Bias));
                const runnerBias = Math.abs(parseFloat(runnerUp.Bias));
                
                const winReasons = [];
                if (champHit > runnerHit) winReasons.push(`tolerance hit-rate (${champ.Hit10}% vs ${runnerUp.Hit10}%)`);
                if (champBias < runnerBias) winReasons.push(`directional bias (${champ.Bias}% vs ${runnerUp.Bias}%)`);
                if (champWape < runnerWape) winReasons.push(`raw WAPE accuracy (${champ.WAPE}% vs ${runnerUp.WAPE}%)`);
                
                let tradeOffText = '';
                if (runnerWape < champWape) {
                    tradeOffText = ` However, runner-up <b>${runnerUp.Model}</b> achieves superior raw accuracy (WAPE ${runnerUp.WAPE}% vs ${champ.WAPE}%); champion leads due to broader tolerance compliance and lower bias penalty.`;
                }
                
                const reasonsStr = winReasons.length > 0 ? winReasons.join(' and ') : 'balanced multi-metric performance';
                decTextEl.innerHTML = `<b>${champ.Model}</b> leads with a composite score of <b>${champ.CompositeScore}</b> &mdash; a <b>${marginLabel.toLowerCase()} (${gapStr} pts)</b> over runner-up <b>${runnerUp.Model}</b> (${runnerUp.CompositeScore}). The champion excels in ${reasonsStr}.${tradeOffText} Evaluated across <b>${champ.ValidWeeks} realized weeks</b> under current scope (<b>${scopeText}</b>).`;
            }
            
            // Render Radar & Metric Comparison Table
            const ruEl = document.getElementById('mc-champ-runnerup');
            if (ruEl) {
                const champWapeWin = parseFloat(champ.WAPE) <= parseFloat(runnerUp.WAPE);
                const champHitWin = parseFloat(champ.Hit10) >= parseFloat(runnerUp.Hit10);
                const champBiasWin = Math.abs(parseFloat(champ.Bias)) <= Math.abs(parseFloat(runnerUp.Bias));
                const champErrWin = parseFloat(champ.MaxErr) <= parseFloat(runnerUp.MaxErr);
                
                ruEl.innerHTML = `
                    <div style="position:relative; height:190px; width:100%;"><canvas id="chart-mc-radar"></canvas></div>
                    <div style="margin-top:12px; border-top:1px solid #E2E8F0; padding-top:10px;">
                      <table style="width:100%; font-size:11.5px; border-collapse:collapse; color:#334155;">
                        <thead>
                          <tr style="border-bottom:1px solid #E2E8F0; text-align:left; color:#64748B; font-weight:700;">
                            <th style="padding:4px 6px;">Metric</th>
                            <th style="padding:4px 6px; text-align:right;">${champ.Model} (Champ)</th>
                            <th style="padding:4px 6px; text-align:right;">${runnerUp.Model} (Runner)</th>
                            <th style="padding:4px 6px; text-align:right;">Winner</th>
                          </tr>
                        </thead>
                        <tbody>
                          <tr style="border-bottom:1px solid #F1F5F9;">
                            <td style="padding:5px 6px; font-weight:600;">WAPE %</td>
                            <td style="padding:5px 6px; text-align:right; font-weight:${champWapeWin?'700':'400'}; color:${champWapeWin?'#047857':'#334155'};">${champ.WAPE}%</td>
                            <td style="padding:5px 6px; text-align:right; font-weight:${!champWapeWin?'700':'400'}; color:${!champWapeWin?'#047857':'#334155'};">${runnerUp.WAPE}%</td>
                            <td style="padding:5px 6px; text-align:right; font-weight:700; color:${champWapeWin?'#047857':'#D97706'};">${champWapeWin ? 'Champion ▲' : 'Runner-Up ▼'}</td>
                          </tr>
                          <tr style="border-bottom:1px solid #F1F5F9;">
                            <td style="padding:5px 6px; font-weight:600;">Hit10 %</td>
                            <td style="padding:5px 6px; text-align:right; font-weight:${champHitWin?'700':'400'}; color:${champHitWin?'#047857':'#334155'};">${champ.Hit10}%</td>
                            <td style="padding:5px 6px; text-align:right; font-weight:${!champHitWin?'700':'400'}; color:${!champHitWin?'#047857':'#334155'};">${runnerUp.Hit10}%</td>
                            <td style="padding:5px 6px; text-align:right; font-weight:700; color:${champHitWin?'#047857':'#D97706'};">${champHitWin ? 'Champion ▲' : 'Runner-Up ▼'}</td>
                          </tr>
                          <tr style="border-bottom:1px solid #F1F5F9;">
                            <td style="padding:5px 6px; font-weight:600;">Bias %</td>
                            <td style="padding:5px 6px; text-align:right; font-weight:${champBiasWin?'700':'400'}; color:${champBiasWin?'#047857':'#334155'};">${champ.Bias}%</td>
                            <td style="padding:5px 6px; text-align:right; font-weight:${!champBiasWin?'700':'400'}; color:${!champBiasWin?'#047857':'#334155'};">${runnerUp.Bias}%</td>
                            <td style="padding:5px 6px; text-align:right; font-weight:700; color:${champBiasWin?'#047857':'#D97706'};">${champBiasWin ? 'Champion ▲' : 'Runner-Up ▼'}</td>
                          </tr>
                          <tr>
                            <td style="padding:5px 6px; font-weight:600;">Max Err %</td>
                            <td style="padding:5px 6px; text-align:right; font-weight:${champErrWin?'700':'400'}; color:${champErrWin?'#047857':'#334155'};">${champ.MaxErr}%</td>
                            <td style="padding:5px 6px; text-align:right; font-weight:${!champErrWin?'700':'400'}; color:${!champErrWin?'#047857':'#334155'};">${runnerUp.MaxErr}%</td>
                            <td style="padding:5px 6px; text-align:right; font-weight:700; color:${champErrWin?'#047857':'#D97706'};">${champErrWin ? 'Champion ▲' : 'Runner-Up ▼'}</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                `;
                renderRadarChart(champ, runnerUp);
            }
...
function renderRadarChart(champ, runnerUp) {
            ...
            options: {
                ...
                scales: {
                    r: {
                        ...
                        suggestedMin: 0,
                        suggestedMax: 100,
                        ticks: { stepSize: 20, font: { size: 9, color: '#94A3B8' }, backdropColor: 'transparent' }
                    }
                },
                plugins: {
                    legend: {
                        position: 'bottom',
                        labels: { boxWidth: 10, boxHeight: 10, usePointStyle: true, padding: 12, font: { size: 11, family: "'IBM Plex Sans', sans-serif" } }
                    },
                    tooltip: {
    ...
    // 2. Render Compact Comparison Table below Radar Chart (Matching Image 1)
    const container = document.getElementById('mc-pairwise-table');
    if (!container) return;

    const champWape = parseFloat(champ.WAPE), ruWape = parseFloat(runnerUp.WAPE);
    const champHit = parseFloat(champ.Hit10), ruHit = parseFloat(runnerUp.Hit10);
    const champBias = parseFloat(champ.Bias), ruBias = parseFloat(runnerUp.Bias);
    const champMax = parseFloat(champ.MaxErr), ruMax = parseFloat(runnerUp.MaxErr);
    ...
    const rows = [
        {
            ...
            cVal: champ.WAPE + '%',
            rVal: runnerUp.WAPE + '%',
            cWins: champWape <= ruWape,
            winnerLabel: champWape <= ruWape ? 'Champion ▲' : 'Runner-Up ▼'
        },
        {
            label: 'Hit10 %',
            cVal: champ.Hit10 + '%',
            rVal: runnerUp.Hit10 + '%',
            cWins: champHit >= ruHit,
            winnerLabel: champHit >= ruHit ? 'Champion ▲' : 'Runner-Up ▼'
        },
        {
            label: 'Bias %',
            cVal: champ.Bias + '%',
            rVal: runnerUp.Bias + '%',
            cWins: Math.abs(champBias) <= Math.abs(ruBias),
            winnerLabel: Math.abs(champBias) <= Math.abs(ruBias) ? 'Champion ▲' : 'Runner-Up ▼'
        },
        {
            label: 'Max Err %',
            cVal: champ.MaxErr + '%',
            rVal: runnerUp.MaxErr + '%',
            cWins: champMax <= ruMax,
            winnerLabel: champMax <= ruMax ? 'Champion ▲' : 'Runner-Up ▼'
        }
    ];

    let html = `<table class="lb" style="font-size:12px; margin-top:8px;">
        <thead><tr>
            <th style="text-align:left;">Metric</th>
            <th class="num" style="min-width:90px;">${champ.Model}<br><span style="font-size:10px;font-weight:400;color:#64748B;">(Champ)</span></th>
            <th class="num" style="min-width:90px;">${runnerUp.Model}<br><span style="font-size:10px;font-weight:400;color:#64748B;">(Runner)</span></th>
            <th style="text-align:center; min-width:80px;">Winner</th>
        </tr></thead><tbody>`;

    rows.forEach(r => {
        const champStyle = r.cWins ? 'color:#059669; font-weight:700;' : 'color:#334155;';
        const ruStyle = !r.cWins ? 'color:#059669; font-weight:700;' : 'color:#334155;';
        const winnerStyle = r.cWins
            ? 'color:#059669; font-weight:700;'
            : 'color:#D97706; font-weight:700;';

        html += `<tr>
            <td style="font-weight:600; color:#475569;">${r.label}</td>
            <td class="num" style="${champStyle}">${r.cVal}</td>
            <td class="num" style="${ruStyle}">${r.rVal}</td>
            <td style="text-align:center; ${winnerStyle}">${r.winnerLabel}</td>
        </tr>`;
    });
...
function renderContribChart(results) {
    ...
    // Top Score Totals Plugin for Data Labels above stacked bars (Safe implementation)
            ...
            try {
                ...
                top5.forEach((r, i) => {
                    ...
                    if (element && typeof element.x === 'number' && typeof element.y === 'number' && !isNaN(element.x) && !isNaN(element.y)) {
                        ctx.fillText(r.CompositeScore, element.x, element.y - 6);
                    }
                });
                ctx.restore();
            } catch (err) {
                console.warn('stackedTotals plugin warning:', err);
            }
        }
    };
        ...
        options: {
            ...
            scales: {
                x: {
                    stacked: true,
                    grid: { display: false },
                    title: { display: true, text: 'Model', font: { size: 11, color: '#8A94A3' } },
                ...
                y: {
                    ...
                    beginAtZero: true,
                    max: 105,
                    title: { display: true, text: 'Composite Score (0-100)', font: { size: 11, color: '#8A94A3' } }
                }
            },
            plugins: {
                legend: { position: 'top', labels: { boxWidth: 8, boxHeight: 8, usePointStyle: true, padding: 8, font: { size: 10 } } },
                tooltip: {
                    backgroundColor: 'rgba(16, 27, 51, 0.95)',
...
function renderModelChampionCharts(results) {
    ...
    // 1. WAPE vs Hit10 Scatter (Exact User Design)
    ...
    if (scatterCtx) {
        ...
        families.forEach(fam => {
            ...
            datasets.push({
                label: fam,
                data: famResults.map(r => ({ x: parseFloat(r.WAPE), y: parseFloat(r.Hit10), r: 5, model: r.Model, weeks: r.ValidWeeks })),
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        ...
                        cornerRadius: 8,
                        callbacks: {
                            label: (ctx) => `${ctx.raw.model}: WAPE ${ctx.raw.x}%, Hit10 ${ctx.raw.y}% · ${ctx.raw.weeks} wks`
                        }
                    }
                },
                scales: {
...
function renderBusinessContext() {
    try { renderBC_Tier1Exceptions(); renderHeatmap(); renderWaterfall(); } catch(e){ console.error(e); }
    let target = RAW_LEVEL0;
    const filters = window.CURRENT_FILTERS.filters;
    const levelName = window.CURRENT_FILTERS.levelName;
    const nodeName = window.CURRENT_FILTERS.nodeName;
...
function downloadModelLeaderboardCSV() {
    const results = window.CURRENT_MC_RESULTS;
    if (!results || !results.length) {
        alert('No Model Champion data available to export.');
        return;
    }
    const headers = ['Rank','Model','CompositeScore','WAPE_Pct','Bias_Pct','MaxErr_Pct','Hit10_Pct','ValidWeeks'];
    const rows = [headers.join(',')];
    results.forEach((r, idx) => {
        rows.push([idx+1, '"'+r.Model+'"', r.CompositeScore, r.WAPE, r.Bias, r.MaxErr, r.Hit10, r.ValidWeeks].join(','));
    });
    const blob = new Blob([rows.join('\n')], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'Model_Leaderboard_' + new Date().toISOString().slice(0,10) + '.csv';
...
// ==========================================
// BUSINESS CONTEXT TIER-1 INSIGHTS & VISUALS
// ==========================================

function renderBC_Tier1Exceptions() {
    const listEl = document.getElementById('bc-exceptions-list');
    if (!listEl) return;
    
    let target = RAW_LEVEL0 || [];
    const filters = (window.CURRENT_FILTERS && window.CURRENT_FILTERS.filters) || {};
    const levelName = (window.CURRENT_FILTERS && window.CURRENT_FILTERS.levelName) || 'Global';
    const nodeName = (window.CURRENT_FILTERS && window.CURRENT_FILTERS.nodeName) || 'Global';
    ...
    // Analyze segment baselines & forecast gaps
    const segments = {};
    target.forEach(row => {
        const seg = row.Forecast_Name;
        const actualRaw = row.Actual_Offered;
        ...
        if (!segments[seg]) segments[seg] = { actual: 0, mean: meanHist, count: 0, region: row.Region };
        if (actual !== null) {
            segments[seg].actual += actual;
            segments[seg].count += 1;
        }
    ...
    const items = [];
    let severeCount = 0;
    Object.entries(segments).forEach(([seg, data]) => {
        const expected = data.mean * data.count;
        if (expected > 0 && data.actual < expected * 0.85) {
            severeCount++;
            const pct = Math.round((data.actual / expected) * 100);
            if (items.length < 4) {
                items.push(`<li><strong>${seg} (${data.region || 'Global'})</strong>: Realized volume tracked at <strong>${pct}% of baseline</strong> (shortfall of ${Math.round(expected - data.actual).toLocaleString()} units).</li>`);
            }
        }
    });

    if (items.length === 0) {
        listEl.innerHTML = '<div style="font-size:13px; color:#047857; font-weight:600;">✦ No severe volume shortfalls detected across operational queues under current filters.</div>';
    } else {
        listEl.innerHTML = items.join('') + (severeCount > 4 ? `<div style="font-size:11px; color:#64748B; margin-top:4px; font-weight:600;">+ ${severeCount - 4} additional queues operating below 85% baseline.</div>` : '');
    }
}

function renderHeatmap() {
    const container = document.getElementById('bc-heatmap-container');
    if (!container) return;
    ...
    // Group by segment and week
    const segMap = {};
    const weekSet = new Set();

    target.forEach(row => {
        const seg = row.Forecast_Name;
        const wk = row.Week_Ending || row.Fiscal_Week;
        const actualRaw = row.Actual_Offered;
        const actual = (actualRaw !== undefined && actualRaw !== null && actualRaw !== '') ? parseFloat(actualRaw) : null;
        const meanHist = parseFloat(row['Mean (Hist. Contacts) (Last 1 yr.)']) || 0;

        if (wk) weekSet.add(wk);
        if (!segMap[seg]) segMap[seg] = { name: seg, region: row.Region, totalActual: 0, totalMean: 0, weeks: {} };

        if (actual !== null) {
            segMap[seg].totalActual += actual;
            segMap[seg].totalMean += meanHist;
            segMap[seg].weeks[wk] = { actual, mean: meanHist, pct: meanHist > 0 ? (actual / meanHist) * 100 : 100 };
        }
    });

    const sortedWeeks = Array.from(weekSet).sort();
    
    // Sort segments by biggest shortfall percentage
    const sortedSegs = Object.values(segMap).map(s => {
        const totalExp = s.totalMean;
        const pct = totalExp > 0 ? (s.totalActual / totalExp) * 100 : 100;
        return { ...s, totalPct: pct };
    }).sort((a, b) => a.totalPct - b.totalPct).slice(0, 12); // Top 12 worst

    if (sortedSegs.length === 0) {
        container.innerHTML = '<div style="padding:20px; text-align:center; color:#64748B;">No segment data available.</div>';
        return;
    }

    let html = '<table style="width:100%; border-collapse:separate; border-spacing:3px; font-size:11px;">';
    html += '<thead><tr><th style="text-align:left; padding:6px 10px; font-weight:700; color:#334155; min-width:180px;">Forecast Segment</th>';
    sortedWeeks.forEach((w, i) => {
        html += `<th style="text-align:center; padding:6px; font-weight:600; color:#64748B; font-size:10px;">W${i+1}</th>`;
    });
    html += '<th style="text-align:right; padding:6px 10px; font-weight:700; color:#334155;">Avg Realized</th></tr></thead><tbody>';

    sortedSegs.forEach(seg => {
        html += `<tr><td style="padding:6px 10px; font-weight:600; color:#0F172A; white-space:nowrap;">${seg.name} <span style="font-size:10px; font-weight:400; color:#64748B;">(${seg.region || ''})</span></td>`;
        
        sortedWeeks.forEach(w => {
            const cell = seg.weeks[w];
            if (!cell) {
                html += '<td style="background:#F1F5F9; border-radius:3px;"></td>';
            } else {
                const pct = Math.round(cell.pct);
                let bg = '#059669'; // >120%
                let color = '#FFFFFF';
                if (pct < 70) { bg = '#991B1B'; color = '#FFFFFF'; }
                else if (pct < 85) { bg = '#DC2626'; color = '#FFFFFF'; }
                else if (pct < 95) { bg = '#F87171'; color = '#FFFFFF'; }
                else if (pct <= 104) { bg = '#F3F4F6'; color = '#1E293B'; }
                else if (pct <= 120) { bg = '#A7F3D0'; color = '#065F46'; }

                html += `<td style="background:${bg}; color:${color}; text-align:center; padding:6px 2px; border-radius:3px; font-weight:700; font-size:10px;" title="${seg.name} (${w}): ${cell.actual.toLocaleString()} actual vs ${Math.round(cell.mean).toLocaleString()} baseline (${pct}%)">${pct}%</td>`;
            }
        });

        const avgPct = Math.round(seg.totalPct);
        const avgColor = avgPct < 85 ? '#DC2626' : (avgPct < 100 ? '#D97706' : '#059669');
        html += `<td style="text-align:right; padding:6px 10px; font-weight:800; color:${avgColor};">${avgPct}%</td></tr>`;
    });
...
let BC_WATERFALL_CHART = null;
function renderWaterfall() {
    const ctx = document.getElementById('chart-bc-waterfall');
    if (!ctx) return;
    if (BC_WATERFALL_CHART) BC_WATERFALL_CHART.destroy();
    ...
    // Compute total baseline vs actual by region
    let totBaseline = 0;
    let totActual = 0;
    const regionGaps = {};
    ...
    target.forEach(row => {
        ...
        totBaseline += meanHist;
        if (!regionGaps[reg]) regionGaps[reg] = { actual: 0, baseline: 0 };
        regionGaps[reg].baseline += meanHist;

        if (actual !== null) {
            totActual += actual;
            regionGaps[reg].actual += actual;
        }
    });

    const labels = ['Baseline', ...Object.keys(regionGaps), 'Realized Actual'];
    const dataValues = [];
    const bgColors = [];

    dataValues.push(Math.round(totBaseline));
    bgColors.push('#1E293B'); // Baseline bar (navy)

    Object.entries(regionGaps).forEach(([reg, vals]) => {
        const gap = Math.round(vals.actual - vals.baseline);
        dataValues.push(gap);
        bgColors.push(gap >= 0 ? '#059669' : '#B3452B'); // Green if gain, rust if drag
    });

    dataValues.push(Math.round(totActual));
    bgColors.push('#2563EB'); // Final actual bar (blue)

    BC_WATERFALL_CHART = new Chart(ctx, {
        type: 'bar',
        data: {
            labels,
            datasets: [{
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: (ctx) => {
                            const val = ctx.raw;
                            const sign = val > 0 && ctx.dataIndex !== 0 && ctx.dataIndex !== labels.length - 1 ? '+' : '';
                            return ` ${ctx.label}: ${sign}${val.toLocaleString()} contacts`;
                        }
                    }
                }
            },
            scales: {
                ...
                y: { beginAtZero: true, grid: { color: '#EEF1F4' }, ticks: { callback: v => (v/1000).toFixed(0) + 'K' } }
            }
        }
    });
}
